...
def lineShape(x,centroid,gamma,sigma,amplitude,alpha,spShift,spProp, mass=27,
...
def hyperFinePredictionFreeAmps_pseudoVoigt(x,centroid,amplitude,gamma,sigma,alpha,spShift,spProp,Alower=0,Aupper=0,
  ...
  for i in range(len(linePositions)):
    ...
    if len(cec_sim_data)==0: xList=np.array([x0, x0+spShift]); gammaList=gamma*np.ones_like(xList); sigmaList=sigma*np.ones_like(xList)
      ...
      sp_shifts, sp_fractions, broadeningList = generateSidePeakFreqs(mass, laserFreq, x0, originalFractionList, cec_sim_energies, freqOffset=freqOffset, colinearity=colinearity, cecBinning=cecBinning)
      #sp_shifts, sp_fractions, broadeningList = generateSidePeakFreqs(mass, laserFreq, -800, originalFractionList, cec_sim_energies, freqOffset=freqOffset, colinearity=colinearity, cecBinning=cecBinning)
      xList = x0+sp_shifts
      #print(sp_shifts); print(sp_fractions); quit()
      gammaList=gamma*np.ones_like(xList); sigmaList=sigma*np.ones_like(xList)
      sigmaList=np.sqrt(sigmaList**2+broadeningList**2)
      gammaList=np.sqrt(gammaList**2+broadeningList**2)
      if equal_fwhm: gammaList=sigmaList*np.sqrt(2*np.log(2))
      #if equal_fwhm: sigmaList=gammaList/np.sqrt(2*np.log(2))
      # print(x0)
      # print(xList[:4])
...
def fitData(xData, yData, yUncertainty, mass, iNucList, jGround, jExcited, peakModel='pseudoVoigt', transitionLabel='bruhLabelThis', colinearity=True, laserFreq=0,
  ...
  22:[2],
  23:[3.89],
  24:[3, 2.9], #this is a guess. For the isomer: 2.99
  25:[3.6447],
  27:[3.64070]}
  ...
  for k,iNuc in enumerate(iNucList):
    ...
    A1,A2,B1,B2=sympy.symbols(prefix+'Alower '+prefix+'Aupper '+prefix+'Blower '+prefix+'Bupper')
    ...
    params.add(str(A1), value=AupperGuess, vary=not(fixed_Aupper))#TODO TODSFDSAODODO
    ...
    for j in range(len(linePositions)):
      params.add(prefix+"mu_"+str(j), expr=str(linePositions[j]) )
...
def fitAndLogData(mass, targetDirectoryName, iNucList, jGround, jExcited, peakModel='pseudoVoigt', transitionLabel='bruhLabelThis',
  ...
  #TODO: implement equal_fwhm toggle
  if subPath!='':
...
def processData(scanDirec, runs, laserFreq, mass, targetDirectoryName, nuclearSpin, jGround, jExcited, exportSpectrum=False, fitAndLog=True, subPath='', peakModel='pseudoVoigt', fixed_Aratio=False,
  ...
  if exportSpectrum: 
    ...
    energyCorrection=energyCorrection,timeOffset=timeOffset,directoryPrefix=directoryPrefix, cuttingColumn=cuttingColumn)
...
def populateFrame(mass, nuclearSpin, result, prefix='iso0', fixed_Aratio=False, uncorrectedResult=False, scanTime=False):
...
def generateSidePeakFreqs(mass, laserFreq, peakFreq, sp_fractions, cec_sim_list, freqOffset=0, colinearity=True, cecBinning=False):
  ...
  if cecBinning!=False: #if true, we are binning the simulated sidepeaks in bins of width cecBinning, and we need to calculate weighted means for sp_positions, and sum the appropriate fractions
    bins=np.arange(np.min(sp_shifts),np.max(sp_shifts)+2*cecBinning, cecBinning)-cecBinning/2
