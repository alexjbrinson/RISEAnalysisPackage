...
def fitDataIndiv(xData, yData, yUncertainty, mass, iNucList, jGround, jExcited, peakModel='pseudoVoigt', transitionLabel='bruhLabelThis', colinearity=True, laserFreq=0,
  ...
  for k,iNuc in enumerate(iNucList):
    ...
    params.add(f'iso{k}_Aupper', value=AupperGuess, vary=not(fixed_Aupper))
    if fixed_Aratio == False: params.add(f'iso{k}_Alower', value=AlowerGuess, vary=not(fixed_Alower))
    else:                     params.add(f'iso{k}_Alower', expr=str(fixed_Aratio)+f'*iso{k}_Aupper')
    params.add(f'iso{k}_Blower',value=BlowerGuess,vary=(jGround>=1 and iNuc>=1))
    params.add(f'iso{k}_Bupper',value=BupperGuess, vary=(jExcited>=1 and iNuc>=1))
    if k==0:  params.add(f'iso{k}_centroid', value=centroidGuess, vary=True)
    elif k==1:params.add(f'iso{k}_centroid', value=centroidGuess+20, vary=True)
    A1,A2,B1,B2=sympy.symbols(f'iso{k}_Alower iso{k}_Aupper iso{k}_Blower iso{k}_Bupper')
    ...
    for i, line in enumerate(linePositions): 
      ...
      constraintExpr=str(f'iso{k}_centroid + '+str(line))
...
def processData(scanDirec, runs, laserFreq, mass, targetDirectoryName, nuclearSpin, jGround, jExcited, exportSpectrum=False, fitAndLog=True, subPath='', peakModel='pseudoVoigt', fixed_Aratio=False, keepLessIntegratedBins=True,
  ...
  if not os.path.exists(spectrumPath): exportSpectrum=True; fitAndLog=True #override user if spectral data doesn't already exist
  if not os.path.exists(fitPath): fitAndLog=True; print('flag2', fitPath) #override user if fit doesn't already exist
  if exportSpectrum: 
